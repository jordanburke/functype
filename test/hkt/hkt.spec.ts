...
describe("HKT", () => {
  ...
  test("should sequence Option of List to List of Option", () => {
    ...
    expect(resultArray[0]?.get()).toBe(1)
    expect(resultArray[1]?.get()).toBe(2)
    expect(resultArray[2]?.get()).toBe(3)
  })
  ...
  test("should return None when traversing with function returning None", () => {
    const list = List([1, 2, 3])
    const result = HKT.traverse(list, (x: number) => (x === 2 ? Option.none() : Option(x * 2)))
