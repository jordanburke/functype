...
function getUserMessage(user: User): string {
  return Match<User, string>(user)
    // Match nested properties with guards
    .case(
      { role: "admin", age: (n: number) => n >= 18, preferences: { theme: "dark" } },
      "Welcome, dark mode admin!"
    )
...
function reducer(state: State, action: Action): State {
  return Match<Action, State>(action)
    .case(
      { type: "SET_USER" },
      a => ({ ...state, user: (a as { type: "SET_USER"; payload: { id: string; name: string } }).payload, loading: false })
    )
    .case(
      { type: "LOGOUT" },
    ...
    .case(
      ...
      a => ({
        ...state,
        settings: { ...state.settings, [(a as { type: "UPDATE_SETTINGS"; payload: { key: string; value: any } }).payload.key]: (a as { type: "UPDATE_SETTINGS"; payload: { key: string; value: any } }).payload.value }
      })
    )
    // Match specific API methods
    .case(
      { type: "API_REQUEST", method: "GET" },
      () => ({ ...state, loading: true })
    )
    .case(
      { type: "API_REQUEST", method: "DELETE" },
      a => {
...
const eventHandler = Match.struct<DomEvent, void>()
  ...
  .case(
    { type: "keypress", key: "Enter", ctrl: true },
  ...
  .case(
    ...
    e => console.log(`Right-click at (${(e as { type: "click"; x: number; y: number; button: "right" }).x}, ${(e as { type: "click"; x: number; y: number; button: "right" }).y})`)
  )
  // Scroll handling
  .case(
    { type: "scroll" },
...
function validateForm(input: unknown): ValidationResult {
  return Match<unknown, ValidationResult>(input)
    // Type guard first
    .when(
      x => !x || typeof x !== "object",
      { valid: false, errors: ["Invalid input format"] }
    )
    ...
    .when(
      ...
      { valid: false, errors: ["Valid email is required"] }
    )
    .when(
    ...
    .when(
      ...
      { valid: false, errors: ["You must accept the terms"] }
    )
    // Check optional fields if present
    .when(
      x => (x as any).age !== undefined && (
