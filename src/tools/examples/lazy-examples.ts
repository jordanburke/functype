...
export function basicLazyUsage() {
  ...
  // Now it runs
  const result = expensiveComputation.orThrow()

  // Subsequent calls return cached result
  const result2 = expensiveComputation.orThrow() // No recomputation
...
export function lazyTransformations() {
  ...
  return formatted.orThrow() // "Result: 84"
}
...
export function lazyChain() {
  ...
  return step3.orThrow() // Logs: "Step 1", "Step 2", "Step 3", returns 35
}
...
export function circularDependencyExample() {
  ...
  const lazyA = Lazy(() => {
    ...
    return { name: "A", dependsOn: lazyB.orThrow() }
  })
  ...
  return lazyA.orThrow() // { name: "A", dependsOn: { name: "B", value: 42 } }
}
...
export function lazyMemoization() {
  ...
  // First access
  const result1 = memoizedExpensive.orThrow()

  // Second access - uses cached value
  const result2 = memoizedExpensive.orThrow()
