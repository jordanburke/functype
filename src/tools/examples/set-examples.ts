...
export function setTransformations() {
  ...
  const lengths = words.map((word) => word.length).toArray()

  const longWords = words.filter((word) => word.length > 5).toArray() // ["functype"]
...
export function setWithObjects() {
  ...
  const userNames = users.map((user) => user.name).toArray()
