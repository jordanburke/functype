...
import { functypeLookup, functypeExamples, searchFunctypes, getAllFunctypes } from "./functype-lookup"
...
describe("Functype Lookup Tools", () => {
  ...
  describe("searchFunctypes", () => {
    ...
    it("should find types by use case", () => {
      ...
      expect(results.size).toBeGreaterThan(0)
      expect(results.exists(type => 
        type.commonUseCases.some(useCase => 
          useCase.toLowerCase().includes("error")
        )
      )).toBe(true)
    })
  })
