...
- **Versioning + publish:** [Changesets](https://github.com/changesets/changesets) — fully independent per-package versioning. Each changeset bumps only the packages it actually changes; no forced grouping. See [`docs/RELEASE.md`](./docs/RELEASE.md) for the full release runbook including the one-time npm trusted-publisher reconfig.
