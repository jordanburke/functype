...
- **Versioning + publish:** [Changesets](https://github.com/changesets/changesets) — the 5 functype-* packages (`functype`, `functype-os`, `functype-log`, `functype-react`, `functype-mcp-server`) are in a `fixed` group and always bump together to the same version. The two eslint packages (`eslint-config-functype`, `eslint-plugin-functype`) release independently on the `2.100.x` line. See [`docs/RELEASE.md`](./docs/RELEASE.md) for the full release runbook including the one-time npm trusted-publisher reconfig.
