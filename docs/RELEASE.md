...
1. **Two `fixed` Changesets groups — each group bumps in lockstep.**
   ...
   - **Eslint pair** (2 packages): `eslint-config-functype`, `eslint-plugin-functype`. Config + plugin are shipped together and must stay in lockstep. On the `2.100.x` line (intentional offset from the functype family).
   
   The two groups version independently of each other. Configured via `fixed: [[...family...], [...eslint pair...]]` in `.changeset/config.json`. Family-cadence is restored after rule (3) below made it safe to do so.
...
4. **Eslint packages release on their own cadence — but as a pair.** The `2.<functype-minor>.<functype-patch>` mirror with the functype family is gone. They were bumped to `2.100.1` during the reset to put visible distance between their version line and the functype line. The eslint pair stays in lockstep with itself via its own `fixed` group (rule 1).
5. **Pre-publish safety gate (`scripts/check-publish-safety.ts`)** runs before `pnpm -r publish` inside the changesets/action publish step. It refuses to publish if: (a) any package would do a major bump without `ALLOW_MAJOR=<pkg>,...` env authorization, (b) any package would publish a lower version than npm has, (c) any alignment group (functype family or eslint pair) is out of sync — drift introduced outside the changesets bump step (manual edits, conflict resolution, snapshot leakage, dependabot, etc.), or (d) `packages/mcp-server/server.json` drifts from package.json. Catches the cascade class of bug AND any group-misalignment at publish-time, before npm locks the version slot.
