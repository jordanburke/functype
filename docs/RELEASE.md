...
1. **The 5 `functype-*` packages are in a `fixed` Changesets group** and always bump together to the same version. A changeset listing any single member (e.g. `"functype": minor`) causes Changesets to bump ALL 5 to that level. This is family-cadence restored after rule (3) below made it safe to do so. Configured via `fixed: [["functype", "functype-os", "functype-log", "functype-react", "functype-mcp-server"]]` in `.changeset/config.json`. The eslint packages are NOT in the group — they release independently on the `2.100.x` line.
