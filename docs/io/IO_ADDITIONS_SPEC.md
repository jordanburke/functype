 ...
 * ```
 */
fromResult: <D extends Type, E extends Type>(result: { data: D | null; error: E | null }): IO<never, E, D> =>
  result.error ? IO.fail(result.error) : IO.succeed(result.data as D)
````
 ...
 * ```
 */
tryAsync: <A extends Type, E extends Type>(f: () => Promise<A>, onError: (error: unknown) => E): IO<never, E, A> =>
...
): IO<never, E, D> => IO.tryAsync(f, onThrow).flatMap((result) => IO.fromResult(result))
````
